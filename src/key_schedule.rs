...
impl KeySchedule {
    pub fn new(hash: &'static digest::Algorithm) -> KeySchedule {
        ...
        empty_hash[..hash.output_len].clone_from_slice(digest::digest(hash, &[]).as_ref());
    ...
    pub fn input_secret(&mut self, secret: &[u8]) {
        ...
        if self.need_derive_for_extract {
            let derived = self.derive(SecretKind::DerivedSecret, self.get_hash_of_empty_message());
    ...
    pub fn derive_ticket_psk(&self, rms: &[u8], nonce: &[u8]) -> Vec<u8> {
        _hkdf_expand_label_vec(
            &hmac::SigningKey::new(self.hash, rms),
            b"resumption",
            nonce,
            self.hash.output_len,
        )
    ...
    pub fn export_keying_material(
        ...
        _hkdf_expand_label(
            &mut secret[..self.hash.output_len],
            &hmac::SigningKey::new(self.hash, &self.current_exporter_secret),
            label,
            h_empty.as_ref(),
        );
        ...
        h_context[..self.hash.output_len]
            .clone_from_slice(digest::digest(self.hash, context.unwrap_or(&[])).as_ref());
...
fn _hkdf_expand_label(output: &mut [u8], secret: &hmac::SigningKey, label: &[u8], context: &[u8]) {
...
mod test {
    ...
    fn test_vectors() {
        ...
        let got_client_hts = ks.derive(SecretKind::ClientHandshakeTrafficSecret, &hs_start_hash);
        assert_eq!(got_client_hts, client_hts.to_vec());
        assert_eq!(
            derive_traffic_key(hash, &got_client_hts, client_hts_key.len()),
            client_hts_key.to_vec()
        );
        assert_eq!(
            derive_traffic_iv(hash, &got_client_hts, client_hts_iv.len()),
            client_hts_iv.to_vec()
        );

        let got_server_hts = ks.derive(SecretKind::ServerHandshakeTrafficSecret, &hs_start_hash);
        assert_eq!(got_server_hts, server_hts.to_vec());
        assert_eq!(
            derive_traffic_key(hash, &got_server_hts, server_hts_key.len()),
            server_hts_key.to_vec()
        );
        assert_eq!(
            derive_traffic_iv(hash, &got_server_hts, server_hts_iv.len()),
            server_hts_iv.to_vec()
        );

        ks.input_empty();

        let got_client_ats = ks.derive(SecretKind::ClientApplicationTrafficSecret, &hs_full_hash);
        assert_eq!(got_client_ats, client_ats.to_vec());
        assert_eq!(
            derive_traffic_key(hash, &got_client_ats, client_ats_key.len()),
            client_ats_key.to_vec()
        );
        assert_eq!(
            derive_traffic_iv(hash, &got_client_ats, client_ats_iv.len()),
            client_ats_iv.to_vec()
        );

        let got_server_ats = ks.derive(SecretKind::ServerApplicationTrafficSecret, &hs_full_hash);
        assert_eq!(got_server_ats, server_ats.to_vec());
        assert_eq!(
            derive_traffic_key(hash, &got_server_ats, server_ats_key.len()),
            server_ats_key.to_vec()
        );
        assert_eq!(
            derive_traffic_iv(hash, &got_server_ats, server_ats_iv.len()),
